...
The intent preview dialog is **always shown** before executing contract functions. No configuration is required.

**What Happens:**
- Contract execution always shows an intent preview dialog before executing
- Shows contract details, function name, typed arguments, security details (nonce, expiration)
- Displays intent bytes and WebAuthn challenge derivation
- User confirms the intent before execution proceeds
- Execution always uses the backend API (handles WebAuthn, payments, smart wallet routing, etc.)
