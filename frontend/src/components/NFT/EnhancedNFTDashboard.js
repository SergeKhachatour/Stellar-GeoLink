...
const EnhancedNFTDashboard = () => {
  ...
  const fetchNearbyNFTs = useCallback(async () => {
    ...
    setMapLoading(true);
    try {
      console.log('🎯 Enhanced NFT Dashboard: Fetching ALL NFTs globally with location:', userLocation);
      ...
      console.log('🎯 Enhanced NFT Dashboard: Nearby NFTs API response:', {
      ...
      console.log('🎯 Enhanced NFT Dashboard: Processed NFTs:', {
  ...
  const createSingleMarker = useCallback((nft, map, nftIndex = 0, markersRef = null) => {
    try {
      ...
      if (!imageUrl || imageUrl === 'https://via.placeholder.com/48x48?text=NFT') {
        console.warn(`⚠️ Enhanced NFT Dashboard: NFT ID ${nft.id} missing image data:`, {
          server_url: nft.server_url,
          ipfs_hash: nft.ipfs_hash,
          image_url: nft.image_url,
          nft_upload_id: nft.nft_upload_id,
      ...
      console.log(`🎯 Enhanced NFT Dashboard: NFT ID ${nft.id} image URL:`, {
        server_url: nft.server_url,
        ipfs_hash: nft.ipfs_hash,
        image_url: nft.image_url,
